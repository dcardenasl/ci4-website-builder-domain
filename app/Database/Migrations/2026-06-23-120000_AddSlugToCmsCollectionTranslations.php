...
class AddSlugToCmsCollectionTranslations extends Migration
{
    public function up(): void
    {
        ...
        $hasSlug = $columnExists && (int) $columnExists->getRowArray()['cnt'] > 0;
        ...
        $sourceExpression = 'c.collection_key';
        if ($prefixColumnExists && (int) $prefixColumnExists->getRowArray()['cnt'] > 0) {
            $sourceExpression = 'TRIM(BOTH \'/\' FROM c.url_prefix)';
        }
        ...
        if ($nullableCheck && (int) $nullableCheck->getRowArray()['cnt'] > 0) {
            $db->query("ALTER TABLE `cms_collection_translations`

...
### Changed
- **`page_type` narrowed to system-behavior values** — `CmsEnums::PAGE_TYPE`, the `cms_pages` migration, and `PageModel` validation drop the editorial labels `about`, `history`, `events`, `components`, `media`, `portfolio`; those distinctions now live in page content and slug instead of a separate field. Existing rows are normalized to `generic` via migration `NormalizeCmsPageTypes`; `PageCreateRequestDTO` now defaults `page_type` to `generic` when omitted instead of requiring it; `SiteAboutPageSeeder`/`SiteHistoryPageSeeder`/`SiteComponentsPageSeeder`/`SiteMediaPageSeeder` locate their demo page by slug instead of the now-removed `page_type` value.
